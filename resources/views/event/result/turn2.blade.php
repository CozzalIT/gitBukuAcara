<div class="col-lg-12 group-seri2">
  <form class="" action="{{ route('addResult') }}" id="formTurn1" method="post">
    {{ csrf_field() }}
    <div class="panel panel-default">
      <div class="panel-body">
        <button type="submit" class="btn btn-primary " name="button" id="save-seri2" style="margin:5px;">Simpan</button>
        <hr>
        <table data-toggle="table" data-url="#"  data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1" data-pagination="true" data-sort-order="desc">
          ...
          <tbody>
            @for ($i=1; $i < 9; $i++)
              @php
                if ($participant = App\Event::showParticipant($event[0]->id, 2, $i) == null) {
                  $participant = null;
                }else{
                  $participant = App\Event::showParticipant($event[0]->id, 2, $i)[0];
                }
              @endphp
              <tr>
                <td>{{ $i }}</td>
                ...
                <td>
                  @if ($participant != null)
                    <input class="input-time2{{$i}}" type="text" name="time[{{$i}}]" value="{{ ($participant->result_time != null) ? "0".$participant->result_time : " " }}" style="border:none; font-weight:bold;" placeholder="-- : -- : ---" data-validation="required">
                    ...
                    <script>
                      var cleaveNumeral = new Cleave('.input-time2{{$i}}', {
                        numericOnly: true,
                        delimiters: [':', ':'],
                        blocks: [2, 2, 3]
                      });
